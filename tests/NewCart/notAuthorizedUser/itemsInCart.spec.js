...
test ('Отображение товаров в корзине', async function ({browser}){
    ...
    await pageCart.goto('https://wa2-dev2.hvalwaters.ru/test_card/index.html');
    ...
    await pageCart.waitForResponse('https://wa2-dev2.hvalwaters.ru/test_api_cart_fjio3we2f2/cart/item/582/');
    await expect(totalAmauntItem.getByText('1 095')).toBeVisible(); //почему-то это работает, в трессеровке выводится эта цифра, а при запущенном тесте выводится другая цена
    
    console.log(await expect(totalAmauntItem.getByText('1 095')).toBeVisible()); //выводится undefined
    
    const deleteItem = pageCart.locator('[data-testid="DeleteOutlineRoundedIcon"]');
    await expect(deleteItem).toBeVisible();
    await deleteItem.click();
    /*const emptyCart = pageCart.locator('[xpath="1"]');
    await expect(emptyCart).toHaveText('Ваша корзина пуста. ');*/
})